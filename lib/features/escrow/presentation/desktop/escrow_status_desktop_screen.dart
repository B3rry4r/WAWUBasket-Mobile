...
/// Desktop-web layout for the shared escrow detail screen. Mirrors the mobile
/// [EscrowStatusScreen] data loading, escrow status logic, money/qty
/// formatting, and navigation exactly — only the layout is re-flowed into a
/// centered status column inside the customer web chrome.
class EscrowStatusDesktopScreen extends StatelessWidget {
  const EscrowStatusDesktopScreen({super.key, required this.orderId});
            ...
            WBCard(
              child: Column(
                children: [
                  _TotalRow(
                    label:
                        '${order.quantityKg} kg × ${wbNaira(order.pricePerKgNaira)}',
