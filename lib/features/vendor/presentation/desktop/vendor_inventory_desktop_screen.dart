...
/// Desktop-web layout for the vendor inventory screen.
///
/// PUSHED route — wrapped in [OperatorDesktopScaffold] so the vendor sidebar
/// stays visible. Re-lays the mobile vertical list into a capped dashboard: a
/// page header with the title + "Add item" action on the right, the low-stock
/// alert banner, the same filter tags, and the inventory cards in a comfortable
/// multi-column grid. Reuses [VendorInventoryController] + the [InventoryItem]
/// model and mirrors the mobile data loading, filtering, status logic, money /
/// number formatting and the add / update / new-batch sheet flows precisely —
/// this is layout only.
class VendorInventoryDesktopScreen extends StatefulWidget {
  const VendorInventoryDesktopScreen({super.key});

  @override
  State<VendorInventoryDesktopScreen> createState() =>
      _VendorInventoryDesktopScreenState();
}

class _VendorInventoryDesktopScreenState
    extends State<VendorInventoryDesktopScreen> {
  String _filter = 'all';
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(context.l10n.vendorInventoryTitle,
                    ...
                    WBButton(
                      label: context.l10n.vendorInventoryAddItem,
                ...
                if (items.isEmpty)
                  const WBEmptyState(
                    ...
                    sub: 'Use Add item to add your first item.',
                  )
                else
                  GridView.builder(
                    shrinkWrap: true,
