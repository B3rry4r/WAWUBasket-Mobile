...
/// Desktop-web layout for the shared operator Account tab (vendor, trader,
/// agent — and later rider/driver). Body content only: the role shell already
/// wraps it in [OperatorDesktopScaffold], so there's no sidebar/top bar here.
///
/// Mirrors [OperatorAccountScreen] exactly — same controllers, role config,
/// name/handle/stat resolution, avatar upload, sign-out sheet, and account-menu
/// sections — re-laid-out into a master rail (profile summary + stats) beside
/// the menu sections, matching [ProfileDesktopScreen]'s dashboard density.
class OperatorAccountDesktopScreen extends StatefulWidget {
  const OperatorAccountDesktopScreen({super.key, required this.role});
  final AppRole role;

  @override
  State<OperatorAccountDesktopScreen> createState() =>
      _OperatorAccountDesktopScreenState();
}

class _OperatorAccountDesktopScreenState
    extends State<OperatorAccountDesktopScreen> {
  ...
  /// Build the three stat tiles from API stats. Falls back to '0' when stats
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // Master rail: profile summary + stats.
                      SizedBox(
                        width: 340,
                      ...
                      // Detail: the account menu sections.
                      Expanded(
                        child: Column(
                          children: [
  ...
  Widget _buildSummaryCard(
    ...
    final displayUrl = _localAvatarUrl ?? avatarUrl;
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                _StatTile(stat: stats[0]),
                _StatTile(stat: stats[1]),
                _StatTile(stat: stats[2], last: true),
              ],
            ),
          ),
        ],
  ...
  /// e.g. "Active: Customer · Vendor · Rider", gives a one-line picture
