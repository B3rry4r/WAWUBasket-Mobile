...
/// Desktop-web layout for [AgentSyncScreen]. Pushed operator screen: wraps its
/// body in [OperatorDesktopScaffold] so the agent sidebar stays visible. Mirrors
/// the mobile data loading (transactions / traders / payouts notifiers), the
/// local sync progress state, conflict-resolver chaining, formatting and
/// navigation exactly; only the layout differs — a page header above a
/// two-column body (the pending-counts card left, the sync action panel right).
class AgentSyncDesktopScreen extends StatefulWidget {
  const AgentSyncDesktopScreen({super.key});

  @override
  State<AgentSyncDesktopScreen> createState() => _AgentSyncDesktopScreenState();
}

class _AgentSyncDesktopScreenState extends State<AgentSyncDesktopScreen> {
  double _progress = 0;
  bool _syncing = false;
  bool _done = false;
          ...
          builder: (_, txns, _) => ValueListenableBuilder(
            ...
            builder: (_, traders, _) => ValueListenableBuilder(
              ...
              builder: (_, payouts, _) {
                ...
                final pendingPayouts = payouts.where((p) => !p.synced).length;
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _SyncRow(
                                ...
                                value: _done
                                    ? 'Just now'
                                    : AgentController.instance.lastSyncAt !=
                                            null
                                        ? _fmtAgo(AgentController
                                            .instance.lastSyncAt!)
                                        : 'Never',
...
/// The right-hand action column: live progress, the done confirmation, the
/// primary sync button and the sign-out affordance — same states and behaviour
/// as the mobile screen's footer.
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder(
      ...
      builder: (_, conflicts, _) {
        final c = conflicts.where((c) => c.id == conflictId).firstOrNull;
        if (c == null) {
      ...
      child: Column(
        ...
        children: [
          ...
          WBButton(
            ...
            variant:
                emphasised ? WBButtonVariant.ghost : WBButtonVariant.secondary,
            onPressed: onKeep,
          ),
        ],
      ),
