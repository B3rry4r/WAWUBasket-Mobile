...
| H5 | Vendor ratings in catalog | `Vendor.fromJson` fallback is `4.6` for all vendors with no ratings | Should show "New" badge or no stars when rating is 0 / no reviews |
...
| M4 | Payment method picker | `top_up_screen.dart:250` snackbar "Method picker, coming soon" | Implement card/bank selector or remove the picker button |
...
| L5 | Checkout delivery fee | Still ₦600 flat; should use vendor's `deliveryFee` field from catalog | `Vendor.fromJson` already parses `deliveryFee`; thread it to checkout |
